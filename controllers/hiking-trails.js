...
module.exports = {
  ...
  getTrails: async (req, res) => {
      try {
          ...
          res.render('profile.ejs', {trails: TrailItems, user: req.user})
      } catch(err) {
          console.error(err)
      }
  },
  ...
  getTrailById: async (req, res) => {
      ...
      try {
          ...
          res.render('trail.ejs', { trail: trail, user: req.user})
      } catch(e) {
          console.error(e)
      }
  },
  ...
  putTrail: async (req, res) => {
      try {
          const trail = await Trail.findOneAndUpdate({_id: req.params.id}, req.body)
          res.redirect(`/hiking-trails/${trail._id}`)
      } catch (err) {
          console.error(err)
      }
  ...
  deleteTrail: async (req, res) => {
      try {
          ...
          await cloudinary.uploader.destory(trail.cloudinaryId)
